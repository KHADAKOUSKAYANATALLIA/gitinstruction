...
10. Копируем вторую строчку – ENTER;
...
>option (enter for default): выбираем 1.
...
5. Копируя адрес (ссылку) репозитория переносим на свой локальный компьютер в VSC - команда *git clone ссылка с нового репозитория* (FORK);
6. ВАЖНО ПОМНИТЬ, что скопировав в VSC нужно ПЕРЕЙТИ на нужный нам файл - команда *cd название нужной нам папки*;
...
17. *git commit -m «»*;
18. Для того чтобы направить то, что мы хотим предложить автору репозитория, необходимо сначала перекинуть в НАШ РЕПОЗИТОРИЙ путем ввода команды *git push* – выдаст «ошибку», т.к. мы пытаемся перенести изменения в наш репозиторий не из основной ветки, а из второстепенной;
